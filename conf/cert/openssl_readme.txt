...
openssl genrsa -out tiaonaer.key 2048    # private key in cleartext

#encrypt the private key with aes 256 algo
openssl rsa -aes256 -in tiaonaer.key -out tiaonaer-aes256.key -passout pass:tiaonaer

2.1.2 OR: encrype it while generating the private key 
(Note: -passout should be placed before 2048)
openssl genrsa -aes256 -out tiaonaer-aes256.key -passout pass:tiaonaer 2048


2.2 create request for certificate
# generate certificate request with extensions in config file
openssl req -new -key tiaonaer-aes256.key -out tiaonaer-request.csr -subj /C=CN/ST=SH/L=SH/O=ZhiSheng\ Inc/OU=WebService/CN=www.tiaonaer.com -extensions v3_req -config ./openssl.cnf -passin pass:tiaonaer

# if request an intermediate CA certificate, e.g can again sign other certificate request
#openssl req -new -key tiaonaer-aes256.key -out tiaonaer-inter-ca-request.csr -subj /C=CN/ST=SH/L=SH/O=ZhiSheng\ Inc/OU=DEV/CN=www.tiaonaer.com -extensions v3_ca -extensions v3_req -config ./openssl.cnf -passin pass:tiaonaer

# to view the request
openssl req -text -noout -in tiaonaer-request.csr


3. sign the request
#openssl ca -in tiaonaer-request.csr -out tiaonaer.crt -cert CARoot2048.crt -keyfile CARoot2048.key -days 1826 -policy policy_anything -config ./openssl.cnf -passin pass:root1234

openssl x509 -req -days 3650 -in ../jks/tiaonaer.csr -CA CARoot2048.crt -CAkey CARoot2048.key -CAcreateserial -out  tiaonaer.x509.crt -extfile ./openssl.cnf -passin pass:root1234
openssl x509 -text -noout -in tiaonaer.x509.crt 


4. generte certificate chain
# this is not accepted by tomcat ????
#openssl pkcs12 -export -in tiaonaer.x509.crt -inkey tiaonaer-aes256.key -out tiaonaer-chain.p12 -name tomcat  -CAfile  CARoot2048.crt -caname root  -chain -passin pass:tiaonaer -passout pass:tiaonaer
